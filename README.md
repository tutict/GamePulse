...
- 完成的研究报告可导出为 Word `.docx` 或 PDF，包含结论、情绪比例、主题、覆盖说明和带 `[E#]` 引用的证据附录。

### Android

- 独立 React + Capacitor 客户端，本地 SQLite、Keystore-backed 凭据和同一套研究报告界面。
- 支持本地研究历史、证据纠正、远程模型追问和高级项目包导入导出。
- Word 报告通过系统分享，PDF 通过 Android 系统打印面板保存，中文字体与 A4 分页由系统 WebView 处理。
...
Android 构建需要 JDK 21 和 Android SDK。仓库 TypeScript 工具链精确锁定为正式版 `7.0.2`。
...
截至 2026-07-17，根测试、全仓类型检查、Windows/Android Web 生产构建和 Android 原生 Java 编译均通过；桌面端已完成 preload、独立原生 SQLite、Steam 评测、Chromium 页面采集、失败来源降级、引用一致性、原生主题同步、自动模型发现、Word/PDF 报告导出和 390px 窄屏布局的本机冒烟验证。
