...
- 未配置模型时仍可生成本地规则报告和证据式追问摘要；配置模型后，追问通过 OpenAI-compatible 或 Ollama 流式生成。
...
截至 2026-07-15，根测试、全仓类型检查和 Windows/Android/Web 生产构建均通过；桌面端已完成 preload、独立原生 SQLite、Steam 评测、Chromium 页面采集、失败来源降级、引用一致性和 390px 窄屏布局的本机冒烟验证。
